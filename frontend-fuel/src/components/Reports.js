...
const Reports = () => {
  ...
  const fetchReports = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.FUEL.REPORTS);
      ...
      const reportsWithNames = await Promise.all(reportsData.map(async (report) => {
        try {
          const routeResponse = await axios.get(API_ENDPOINTS.ROUTES.BY_ID(report.routeId));
  ...
  const handleSubmitUpdateStatus = async () => {
    try {
      await axios.post(API_ENDPOINTS.FUEL.REPORT_STATUS(selectedReport.id), {
        estado: newEstado
      });
  ...
  const fetchSystemStats = async () => {
    ...
    try {
      const [usersRes, driversRes, vehiclesRes, routesRes, fuelReportsRes] = await Promise.all([
        ...
        axios.get(API_ENDPOINTS.FUEL.REPORTS)
      ]);
            ...
            {loadingStats ? (
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={6} md={4}>
                  <Card sx={{ height: '100%' }}>
                    <CardContent sx={{ textAlign: 'center' }}>
                      <DriveEta sx={{ fontSize: 40, color: 'primary.main', mb: 1 }} />
                      <Typography variant="h4" color="primary">
                        {systemStats.drivers}
                      </Typography>
                      <Typography variant="body1" color="text.secondary">
